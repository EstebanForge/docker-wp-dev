...
collect_configuration() {
  ...
  # WordPress Domain
  prompt_with_default "Local domain name" "wp.local" "WP_DOMAIN"
  # Ensure .local TLD if not provided
  if [[ "${WP_DOMAIN}" != *.* ]]; then
    WP_DOMAIN="${WP_DOMAIN}.local"
...
display_final_instructions() {
  ...
  echo ""
  echo -e "Next steps:"
  echo -e "  1. Start the environment: ${BLUE}./start.sh${NC}"
